...
import '../../../Core/services/fincra_payout_service.dart';
...
class _WithdrawFundsScreenState extends ConsumerState<WithdrawFundsScreen> {
  ...
  final _payoutService = FincraPayoutService();
  ...
  void initState() {
    super.initState();
    _selectDefault(ref.read(walletDashboardProvider).paymentMethods);
  }

  void _selectDefault(List<PaymentMethod> methods) {
    if (methods.isEmpty) {
  ...
  Future<void> _openPaymentMethods() async {
    ...
    if (!mounted) return;
    setState(() => _selectDefault(ref.read(walletDashboardProvider).paymentMethods));
  }
  ...
  Future<void> _submit() async {
    ...
    final balance = ref.read(walletDashboardProvider).balanceFor('GHS');
    ...
    if (method == null) {
      setState(() => _error = 'Add a Mobile Money method before withdrawing.');
      return;
    }
    ...
    try {
      final transaction = await _payoutService.initiate(
        amount: amount,
        ...
        SnackBar(
          content: Text(
            completed
                ...
                : 'Withdrawal submitted. We will update your activity when it is complete.',
          ),
        ),
      );
