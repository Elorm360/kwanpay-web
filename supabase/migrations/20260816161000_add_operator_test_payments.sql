...
begin
  ...
  case normalized_code
    when 'ecg' then
      operator_name := 'ECG';
    when 'ghana_water' then
      operator_name := 'Ghana Water';
    when 'dstv' then
      operator_name := 'DStv';
    when 'gotv' then
      operator_name := 'GOtv';
    when 'mtn_airtime' then
      operator_name := 'MTN Airtime';
    when 'telecel_airtime' then
      operator_name := 'Telecel Airtime';
    when 'airteltigo_airtime' then
      operator_name := 'AirtelTigo Airtime';
    when 'stc' then
      operator_name := 'STC';
    else
  ...
  if normalized_code in ('mtn_airtime', 'telecel_airtime', 'airteltigo_airtime') then
    normalized_account := regexp_replace(
    ...
    if length(normalized_account) = 10 and left(normalized_account, 1) = '0' then
      normalized_account := '233' || substring(normalized_account from 2);
    elsif length(normalized_account) = 9 then
      normalized_account := '233' || normalized_account;
    end if;

    if length(normalized_account) <> 12 or left(normalized_account, 3) <> '233' then
      raise exception 'Enter a valid Ghana mobile number';
    end if;
  else
